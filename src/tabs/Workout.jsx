  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-surface rounded-2xl p-4 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            {photos.map((url, i) => (
              <div key={i} className="relative group aspect-square">
                <img src={url} alt="Workout photo" className="w-full h-full object-cover rounded-xl" />
                <button
        ...
        {/* Two inputs: one for gallery (accept=image/*), one for all files (no accept = shows file manager with Screenshots) */}
  ...
  const handlePhotoImport = async (e) => {
    ...
    try {
      ...
      if (data.error) {
        ...
        alert("Couldn't read a workout from that image. Try a photo of a whiteboard or handwritten session plan.")
      }
    } catch (err) {
      alert('Could not parse image: ' + err.message)
    } finally {
